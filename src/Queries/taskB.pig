--read pages
access_logs = LOAD 'hdfs://localhost:9000/user/cs4433/project2/access_logs.csv' USING PigStorage(',') as (AccessID:chararray, ByWho:chararray, WhatPage:chararray, TypeOfAccess:chararray, AccessTime:chararray);

group_data = GROUP access_logs by WhatPage;

access_count = FOREACH group_data GENERATE group AS WhatPage, COUNT(access_logs.AccessID) AS AccessCount;
