    ...
    public static class KMeansReducer extends Reducer<Text, Text, Text, Text> {
        ...
        public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
            ...
            if (!convergenceReached) {
                convergenceReached = isConverged(newCentroidX, newCentroidY, key.toString());
            }
        }

        private boolean isConverged(int newCentroidX, int newCentroidY, String currentCentroid) {
            String[] parts = currentCentroid.split(",");
            int oldCentroidX = Integer.parseInt(parts[0]);
            int oldCentroidY = Integer.parseInt(parts[1]);
            int threshold = 1; // You can adjust the convergence threshold as needed

            return Math.abs(newCentroidX - oldCentroidX) <= threshold && Math.abs(newCentroidY - oldCentroidY) <= threshold;
        }

        @Override
        protected void cleanup(Context context) throws IOException, InterruptedException {
            context.write(new Text("Convergence reached: "), new Text(convergenceReached ? "yes" : "no"));
        }
    }
